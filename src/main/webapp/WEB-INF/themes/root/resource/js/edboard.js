...
$.fn.edBoard = function () {
  ...
  this.loadCallback = function (json) {
    ...
    if (numOfEditors > 0) {
      editorsDiv.append("<p>Displaying  " + parseInt(startIndex + 1) + "-" + endIndex +
          " of " + parseInt(numOfEditors) + " Editors.</p>");
    }
  ...
  this.initializeAutoSuggest = function (args) {
    ...
    textbox.keyup(function (eventObj) {
      if (eventObj.keyCode == 13) {
        ...
        console.log($(this).data("ui-autocomplete").menu.options);
        if ($(this).data("ui-autocomplete").menu.options) {
          console.log($(this).data("ui-autocomplete").menu.options[0]);
        }
      }
    });
      ...
      source: function (entry, response) {
        ...
        if (terms.length > 0) {
          ...
          var success_handler = function (json_subjects, json_names) {
              ...
              $.each(subjects, function (index, subject) {
                ...
                if ($.inArray(subject, terms) == -1) {
                  if ((index / 2) < areas_count && index % 2 == 0 && subjects[index + 1] > 0) {
                    if (!subject_title) {
                      subject_title = true;
                      options.push({ label: '<span class="header">Areas of Expertise</span>', type: "html", value: "info"});
                    }
                    var label = subject + " (" + subjects[index + 1] + ")";
                    if (prefix.length > 0) {
                      label = "<b>" + label.substr(0, prefix.length) + "</b>" + label.substr(prefix.length);
                    }
                    options.push({ label: '<span class="item">' + label + '</span>', value: subject, type: "html" });
                  }
                }
              });
            }
                ...
                $.each(names, function (index, name0) {
                  if (index < names_count) {
                    ...
                    if (!name0.ae_last_name) {
                      ...
                      last_first = parts.pop() + ", " + parts.join(" ");
                    }
                    else {
                    ...
                    if ($.inArray('"' + people + '"', terms) == -1) {
                      if (!name_title) {
                        name_title = true;
                        options.push({ label: '<span class="header">People</span>', type:"html", value: "info"});
                      }
                      ...
                      options.push({ label: '<span class="item">' + label + '</span>', value: '"' + people + '"', type: "html"});
                    }
                  }
                });
              }
            ...
            options.push({ label: '<span class="more">keep typing to see more results...</span>', type: "html", value: "info"});
