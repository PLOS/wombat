...
(function ($) {
  ...
  $.fn.floatingNav = function (options) {
    ...
    $('body').on(
        "click", "nav ul." + options.ulClass + " a", function (event) {
          var link = $(this);
          ...
          $('ul.' + options.ulClass + ' li:not(.' + options.classActive + ') ul').addClass('suppress_expansion');

          $('html,body').animate(
              {scrollTop: $('#' + this.hash.substring(1)).offset().top}, 500, // re-enable expansion/collapse behavior of nested list items (for use during page scrolling)
              function () {$('ul.' + options.ulClass + ' li ul').removeClass('suppress_expansion')}
          );
        }
    );
    return this.each(
        function () {
          ...
          var hilite = function () {
            ...
            options.content.find(options.sectionAnchor).each(
                function () {
                  this_a = $(this);
          ...
          var positionEl = function () {
            ...
            var article_top = options.content.offset().top, el_view_out = (win_top > (article_top - options.margin)),  //the top of the element is out of the viewport
