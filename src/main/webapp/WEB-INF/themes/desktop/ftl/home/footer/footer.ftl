</div><#-- Close pagebdy from header.ftl -->
</div><#-- Close pagebdy-wrap from header.ftl -->
<div id="pageftr">
  <div class="ftr-cols cf">
    <div class="col col-1">
      <img src="/images/logo-plos-footer.png" alt="PLOS Logo" class="logo"/>

      <p><a href="/static/releaseNotes">Ambra 2.9.7</a> Managed Colocation provided <br/>by <a
          href="http://www.isc.org/">Internet Systems Consortium</a>.

      <p>

      <div class="nav nav-aux">
        <a href="/static/privacy">Privacy Policy</a> |
        <a href="/static/terms">Terms of Use</a> |
        <a href="http://www.plos.org/advertise/">Advertise</a> |
        <a href="http://www.plos.org/about/media-inquiries/">Media Inquiries</a>
      </div>
    </div>
    <div class="col col-2">
      <p><a href="http://www.plos.org/publications/journals/">Publications</a></p>

      <div class="nav">
        <ul>
          <li><a href="http://plosbiology-rskonnord:8080">PLOS Biology</a></li>
          <li><a href="http://plosmedicine-rskonnord:8080">PLOS Medicine</a></li>
          <li><a href="http://ploscompbiol-rskonnord:8080">PLOS Computational Biology</a></li>
          <li><a href="http://currents.plos.org">PLOS Currents</a></li>
          <li><a href="http://plosgenetics-rskonnord:8080">PLOS Genetics</a></li>
          <li><a href="http://plospathogens-rskonnord:8080">PLOS Pathogens</a></li>
          <li><a href="http://plosone-rskonnord:8080">PLOS ONE</a></li>
          <li><a href="http://plosntds-rskonnord:8080">PLOS Neglected Tropical Diseases</a></li>
        </ul>
      </div>
    </div>
    <div class="col col-3">
      <div class="nav">
        <p><a href="http://www.plos.org">plos.org</a></p>

        <p><a href="http://blogs.plos.org">Blogs</a></p>

        <p><a href="http://www.ploscollections.org">Collections</a></p>

        <p><a href="/feedback/new">Send us feedback</a></p>
      </div>
    </div>
  </div>
</div>

</div><#-- Close page-wrap from header.ftl -->