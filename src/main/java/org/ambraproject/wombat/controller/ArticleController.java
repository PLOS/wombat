...
public class ArticleController extends WombatController {
   ...
   * @param model        data to pass to the view
   * @param site         current site
   * @param correctionId specifies the correction
   * @return path to the template
   * @throws IOException
   */
  ...
  public String renderArticleCorrectionTree(Model model, @PathVariable("site") String site,
                                            @RequestParam("id") String correctionId) throws IOException {
    requireNonemptyParameter(correctionId);
    Map<?, ?> correction = soaService.requestObject(String.format("corrections/" + correctionId), Map.class);
